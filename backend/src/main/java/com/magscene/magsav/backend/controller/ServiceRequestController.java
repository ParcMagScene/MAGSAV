...
public class ServiceRequestController {
    ...
    public ResponseEntity<ServiceRequestDTO> createServiceRequest(@Valid @RequestBody ServiceRequest serviceRequest) {
        ...
        logger.info("[SAV] ServiceRequest sauvegardé: id={}, equipment={}", savedRequest.getId(),
                savedRequest.getEquipment());
        ServiceRequestDTO dto = new ServiceRequestDTO();
    ...
    public Map<String, Object> getSavStats() {
        ...
        stats.put("pending", serviceRequestRepository.countByStatus(ServiceRequestStatus.PENDING));
        stats.put("validated", serviceRequestRepository.countByStatus(ServiceRequestStatus.VALIDATED));
        ...
        // Demandes urgentes (priorité haute et non validées)
        stats.put("urgent",
                serviceRequestRepository.countByPriorityAndStatusNot(Priority.HIGH, ServiceRequestStatus.VALIDATED));
    ...
    public ResponseEntity<ServiceRequest> assignTechnician(
        ...
        return serviceRequestRepository.findById(id)
                .map(request -> {
                    ...
                    // Le statut ne change pas lors de l'assignation
    ...
    public ResponseEntity<ServiceRequest> resolveServiceRequest(
        ...
        return serviceRequestRepository.findById(id)
                .map(request -> {
                    // Les demandes restent en statut VALIDATED
    ...
    public ResponseEntity<Map<String, Object>> validateAndCreateIntervention(@PathVariable Long id) {
        ...
        // Vérifier que la demande est bien au statut OPEN (seules les demandes OPEN
        // peuvent être validées)
        if (demande.getStatus() != ServiceRequest.ServiceRequestStatus.PENDING) {
            String currentStatus = demande.getStatus() != null ? demande.getStatus().getDisplayName() : "inconnu";
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Map.of("error",
                            "Cette demande ne peut pas être validée (statut actuel: " + currentStatus + ")"));
        }
        ...
        intervention.setStatus(ServiceRequest.ServiceRequestStatus.VALIDATED);
