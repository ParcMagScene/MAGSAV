...
public class QRCodeScannerView extends VBox {
    ...
    private HBox createScannerHeaderSection() {
        ...
        sessionBox.setStyle("-fx-background-color: " + UnifiedThemeManager.getInstance().getCurrentUIColor()
                + "; -fx-padding: 8px 12px; -fx-background-radius: 6px;");

        statsLabel.setText(
                String.format("📊 Scannés: %d | Nouveaux: %d | Erreurs: %d", totalScanned, newItemsFound, errorCount));
    ...
    private VBox createScanningSection() {
        ...
        scanningSection.setStyle("-fx-background-color: " + UnifiedThemeManager.getInstance().getCurrentUIColor()
    ...
    private HBox createScanInputBox() {
        ...
        manualCodeField.setStyle(
                "-fx-font-size: 14px; -fx-background-radius: 6px; -fx-border-color: #27ae60; -fx-border-width: 2px; -fx-border-radius: 6px; -fx-padding: 8px;");
    ...
    private VBox createScanResultsBox() {
        ...
        scanResultArea.setStyle("-fx-background-color: " + UnifiedThemeManager.getInstance().getCurrentSecondaryColor()
                + "; -fx-border-color: #27ae60; -fx-border-radius: 4px; -fx-font-family: 'Courier New'; -fx-font-size: 11px; -fx-padding: 8px;");
    ...
    private VBox createInventoryTableSection() {
        ...
        inventorySection.setStyle("-fx-background-color: " + UnifiedThemeManager.getInstance().getCurrentUIColor()
    ...
    private HBox createInventoryFiltersBox() {
        ...
        categoryFilter.getItems().addAll("Toutes catégories", "Audio", "Vidéo", "Éclairage", "Mécanique", "Réseau",
                "Accessoires");
        ...
        statusFilter.getItems().addAll("Tous statuts", "Disponible", "En service", "En réparation", "Hors service",
                "Nouveau");
    ...
    private VBox createEquipmentDetailsSection() {
        ...
        detailsSection.setStyle("-fx-background-color: " + UnifiedThemeManager.getInstance().getCurrentUIColor()
                + "; -fx-padding: 15px; -fx-background-radius: 8px;");
        ...
        equipmentImageView
                .setStyle("-fx-background-color: " + UnifiedThemeManager.getInstance().getCurrentSecondaryColor()
        ...
        equipmentDetailsArea.setStyle("-fx-background-color: "
                + UnifiedThemeManager.getInstance().getCurrentSecondaryColor()
                + "; -fx-border-color: #27ae60; -fx-border-radius: 4px; -fx-font-family: 'Segoe UI'; -fx-font-size: 11px;");
    ...
    private HBox createScannerActionsBar() {
        ...
        actionsBar.getChildren().addAll(newInventoryBtn, generateQRBtn, exportInventoryBtn, syncInventoryBtn,
                resetSessionBtn);
        return actionsBar;
    }
    ...
    private void processScannedCode(String code) {
        ...
        Task<InventoryItem> processTask = new Task<InventoryItem>() {
            ...
            protected InventoryItem call() throws Exception {
                ...
                // CompletableFuture<List<Equipment>> searchFuture =
    ...
    private void displayEquipmentDetails(InventoryItem item) {
        ...
        // Cette méthode pourrait afficher des informations détaillées dans le panneau
        // droit
    }
    ...
    private void loadInventoryItems() {
        // Charger les équipements déjà scannés ou en cours; // Cette méthode pourrait
        // charger les données depuis une session précédente
    }
