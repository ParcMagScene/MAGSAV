...
public class WeekCalendarView extends VBox {
    ...
    private static final int DEFAULT_TOTAL_WIDTH = 1200; // largeur par défaut; // Calcul dynamique de la largeur des colonnes de jours
    private double calculateDayColumnWidth() {
        // Utiliser la largeur réelle du composant ou fallback sur DEFAULT si pas encore layout
    ...
    private GridPane createUnifiedGrid() {
        ...
        // Configuration des colonnes avec largeurs fixes et égales; // Colonne des heures : largeur fixe
        ...
        for (int hour = START_HOUR; hour <= END_HOUR; hour++) {
            ...
            timeLabel.setMaxHeight(Double.MAX_VALUE); // Expansion verticale; // Forcer l'expansion dans la grille et alignement en haut
    ...
    private void createHeaderRow(GridPane grid) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            dayLabel.setMaxWidth(Double.MAX_VALUE); // Même expansion possible; // Highlight aujourd'hui avec couleur plus discrète
    ...
    /**
     * Met à jour les largeurs des headers et cellules pour maintenir l'alignement avec les colonnes
     */
    ...
    private class TimeSlot extends StackPane {
        ...
        public TimeSlot(LocalDate date, int hour, int dayIndex) {
            ...
            setMaxWidth(Double.MAX_VALUE); // Permettre l'expansion comme les colonnes; // Hauteur : minimum fixe mais peut s'étendre
        ...
        private void setupMouseHandlers() {
            ...
            setOnMouseReleased(e -> {
                if (WeekCalendarView.this.isSelecting && WeekCalendarView.this.selectionStart != null && WeekCalendarView.this.selectionEnd != null) {
                    WeekCalendarView.this.isSelecting = false;
                    
                    // Ajuster à la précision de 15 minutes
                    LocalDateTime adjustedStart = WeekCalendarView.this.adjustToQuarterHour(WeekCalendarView.this.selectionStart);
                    LocalDateTime adjustedEnd = WeekCalendarView.this.adjustToQuarterHour(WeekCalendarView.this.selectionEnd.plusMinutes(15));
    ...
    public static class EventBlock extends StackPane {
        ...
        private void initializeEventBlock() {
            ...
            setMinWidth(30); // Largeur minimum pour rester lisible; // Contenu de l'événement avec contraintes de taille
        ...
        private void setupEventHandlers() {
            ...
            setOnMouseEntered(e -> {
                setStyle("-fx-effect: dropshadow(gaussian, rgba(0,0,0,0.6), 8, 0, 0, 3);");
            });
            
            setOnMouseExited(e -> {
                setStyle("-fx-effect: dropshadow(gaussian, rgba(0,0,0,0.3), 3, 0, 0, 1);");
            });
        ...
        public String[] getCategoryColors() {
            ...
            // Couleurs FONCÉES et PROFESSIONNELLES par défaut [backgroundColor, borderColor, textColor]
    ...
    public void addEvent(String title, LocalDateTime start, LocalDateTime end, String category) {
        ...
        if (hourIndex >= 0 && hourIndex < (END_HOUR - START_HOUR) && dayIndex >= 0 && dayIndex < 7) {
            ...
            // CONTRAINTES STRICTES : L'événement ne doit JAMAIS affecter la taille des cellules
            ...
            // Recalculer les positions de TOUS les événements de ce jour pour éviter les chevauchements
    ...
    /**
     * Force le recalcul de la largeur des colonnes (à utiliser si le conteneur change de taille)
     */
    ...
    private double calculateActualColumnWidth() {
        ...
        double totalWidth = timeGrid.getWidth() > 0 ? timeGrid.getWidth() : 
                           (getWidth() > 0 ? getWidth() : 900); // Valeur par défaut augmentée; // 8 colonnes au total : 1 colonne d'heures + 7 colonnes de jours
    ...
    private void layoutEventsGoogleStyle(List<EventBlock> dayEvents, double columnWidth, double margin) {
        ...
        // Trier les événements par heure de début, puis par durée (plus longs en premier)
        ...
        for (EventInfo info : eventInfos) {
            ...
            System.out.printf("📍 '%s' → Col=%d, X=%.1f, W=%.1f, Fin=%.1f/%.1f%n", 
                info.event.getTitle().length() > 10 ? info.event.getTitle().substring(0, 10) + "..." : info.event.getTitle(),
    ...
    private boolean eventsOverlap(EventBlock event1, EventBlock event2) {
        // Amélioration : inclure les événements qui se touchent exactement comme des chevauchements
    ...
    private void applyUnifiedEventStyle(EventBlock event, double width) {
        ...
        String unifiedStyle = 
            ...
            "-fx-text-background-color: " + backgroundColor + "; " +      // Override fond du texte; // Effets visuels cohérents
    ...
    private void updateSelectionVisual() {
        // TODO: Implémenter l'affichage de la sélection en cours; // Pour l'instant, on peut utiliser un rectangle semi-transparent
        ...
        if (selectionStart != null && selectionEnd != null) {
            ...
            // TODO: Calculer position et taille basées sur start/end; // Pour l'instant, juste marquer visuellement qu'il y a une sélection
        }
    }
