...
public class CSVImporter {
    ...
    public CSVImporter(DataSource ds, List<String> patterns) {
        ...
        if (patterns == null || patterns.isEmpty()) {
            ...
            this.dateFormats = patterns.stream()
                .map(p -> {
                    try { return DateTimeFormatter.ofPattern(p); } catch (Exception e) { return null; }
                })
                .filter(f -> f != null)
    ...
    /**
     * Import simple d'un CSV Produits (produit, numero_serie, proprietaire, panne, statut, detecteur, date_entree, date_sortie)
     * S'appuie sur la logique dossiers SAV pour réutiliser le même repository.
     */
    ...
    /**
     * Import du CSV principal avec structure :
            ...
            while ((row = rdr.readNext()) != null) {
                ...
                try {
                    ...
                    if (isEmpty(produit) || isEmpty(numeroSerie) || isEmpty(proprietaire)) {
                        System.err.printf("Ligne %d ignorée : champs obligatoires manquants (PRODUIT, N° DE SERIE, PROPRIETAIRE)%n", lineNumber);
                        continue;
                    }
                    ...
                    DossierSAV dossier = new DossierSAV(
                        null, produit, numeroSerie, proprietaire, panne, statut, detecteur,
                        dateEntree, dateSortie, null
                    );
                    
                    dossierSAVRepo.upsertBySerieProprietaire(dossier);
                    importedCount++;
