...
public class AppareilRepository {
  ...
  private Appareil insert(Appareil appareil) throws SQLException {
    String sql =
        "INSERT INTO appareils(client_id, marque, modele, sn, accessoires) VALUES(?,?,?,?,?)";
  ...
  private Appareil update(Appareil appareil) throws SQLException {
    String sql =
        "UPDATE appareils SET client_id=?, marque=?, modele=?, sn=?, accessoires=? WHERE id=?";
    try (Connection conn = ds.getConnection();
  ...
  public List<Appareil> findBySerialNumber(String sn) throws SQLException {
    String sql =
        "SELECT id, client_id, marque, modele, sn, accessoires FROM appareils WHERE sn LIKE ?";
    try (Connection conn = ds.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
  ...
  public List<Appareil> findByClientId(Long clientId) throws SQLException {
    String sql =
        "SELECT id, client_id, marque, modele, sn, accessoires FROM appareils WHERE client_id=?";
    try (Connection conn = ds.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
