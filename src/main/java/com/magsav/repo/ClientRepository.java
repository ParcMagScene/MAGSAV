...
public class ClientRepository {
  ...
  public Client upsertByEmail(Client c) throws SQLException {
    try (Connection conn = ds.getConnection()) {
      ...
      String insert =
          "INSERT OR IGNORE INTO clients(nom, prenom, email, tel, adresse) VALUES(?,?,?,?,?)";
  ...
  public Client findById(Long id) throws SQLException {
    try (Connection conn = ds.getConnection();
        PreparedStatement ps =
            conn.prepareStatement(
                "SELECT id, nom, prenom, email, tel, adresse FROM clients WHERE id=?")) {
      ps.setLong(1, id);
      try (ResultSet rs = ps.executeQuery()) {
        if (rs.next()) {
          return new Client(
  ...
  public List<Client> findAll() throws SQLException {
    ...
    try (Connection conn = ds.getConnection();
        PreparedStatement ps =
            conn.prepareStatement(
                "SELECT id, nom, prenom, email, tel, adresse FROM clients ORDER BY nom, prenom")) {
      try (ResultSet rs = ps.executeQuery()) {
        while (rs.next()) {
          list.add(
              new Client(
  ...
  public List<Client> search(String term) throws SQLException {
    ...
    try (Connection conn = ds.getConnection();
        PreparedStatement ps =
            conn.prepareStatement(
                "SELECT id, nom, prenom, email, tel, adresse FROM clients WHERE lower(nom) LIKE ? OR lower(prenom) LIKE ? OR lower(email) LIKE ? ORDER BY nom, prenom")) {
      ps.setString(1, like);
      ps.setString(2, like);
      ps.setString(3, like);
      try (ResultSet rs = ps.executeQuery()) {
