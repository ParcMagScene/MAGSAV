...
Write-Host "  MAGSAV 3.0 - Test Intégration Backend" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Vérifier que nous sommes dans le bon répertoire
if (!(Test-Path ".\gradlew.bat")) {
    Write-Host "❌ Erreur: gradlew.bat non trouvé" -ForegroundColor Red
    Write-Host "   Exécutez ce script depuis la racine du projet MAGSAV-3.0" -ForegroundColor Yellow
    exit 1
}

Write-Host "📋 Étape 1/4 : Compilation du projet..." -ForegroundColor Green
...
Write-Host "📋 Étape 2/4 : Démarrage du backend Spring Boot..." -ForegroundColor Green
...
Write-Host ""
Write-Host "📋 Étape 3/4 : Vérification du backend..." -ForegroundColor Green
try {
    ...
    Write-Host "⚠️  Backend pas encore prêt, mais on continue..." -ForegroundColor Yellow
}

Write-Host ""
Write-Host "📋 Étape 4/4 : Lancement de l'application Desktop..." -ForegroundColor Green
Write-Host ""
Write-Host "╔════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
...
Write-Host "║  3. Vérifiez le message de statut :                  ║" -ForegroundColor White
Write-Host "║     ✅ 'X demande(s) chargée(s) depuis le backend'    ║" -ForegroundColor Green
Write-Host "║     ⚠️  'Backend indisponible - données test'         ║" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "⚠️  Impossible d'arrêter le backend (PID: $($backendJob.Id))" -ForegroundColor Yellow
}
